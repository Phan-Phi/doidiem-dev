...
export default function GuideListingMenu({ data, title }) {
    ...
    return data.items.map((el, idx) => {
      ...
      return (
        <TitleAccordion
          ...
          onClick={() => {
            router.push(
              `/huong-dan-su-dung/${router.query.slug}/${el.meta.slug}`
            );
          }}
        >
          {el.title}
  ...
  }, [data, router.query.slug, expanded]);
